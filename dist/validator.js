...
function compareStrings(field, extracted, reference) {
    if (!extracted && !reference)
        return null;
    if (!extracted || !reference) {
        return { field, extracted: extracted ?? null, reference: reference ?? null, note: 'One value is missing' };
    }
    const sim = (0, utils_1.stringSimilarity)(extracted, reference);
    if (sim < STRING_SIMILARITY_THRESHOLD) {
        return { field, extracted, reference, note: `Similarity: ${(sim * 100).toFixed(1)}%` };
    }
    return null;
}
function compareNumbers(field, extracted, reference, tolerance = 0.01) {
    if (extracted == null && reference == null)
        return null;
    if (extracted == null || reference == null) {
        return { field, extracted: extracted ?? null, reference: reference ?? null, note: 'One value is missing' };
    }
    if (!(0, utils_1.numbersMatch)(extracted, reference, tolerance)) {
        return { field, extracted: (0, utils_1.round2)(extracted), reference: (0, utils_1.round2)(reference) };
    }
    return null;
}
function matchLineItems(extractedItems, referenceItems) {
    const issues = [];
    const used = new Set();
    for (const refItem of referenceItems) {
        ...
        for (let i = 0; i < extractedItems.length; i++) {
            if (used.has(i))
                continue;
            const sim = (0, utils_1.stringSimilarity)(extractedItems[i].description, refItem.description);
            if (sim > bestSim) {
                bestSim = sim;
                bestIdx = i;
            }
...
function validateInvoice(extracted, reference, sourceFile) {
    if (!reference) {
        return {
    ...
    if (!(0, utils_1.numbersMatch)(computedTotal, extracted.total_amount, 0.10)) {
        issues.push({
            field: 'internal_total_check',
            extracted: (0, utils_1.round2)(computedTotal),
            reference: (0, utils_1.round2)(extracted.total_amount),
